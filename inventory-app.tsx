...
import { Plus, Minus, ArrowLeftRight, Warehouse, Truck, AlertTriangle, X, Check, RotateCcw } from 'lucide-react';
...
export default function InventoryApp() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await window.storage.get('inventory-v1');
        if (res && res.value) {
  ...
  const persist = useCallback(async (next) => {
    try {
      await window.storage.set('inventory-v1', JSON.stringify(next));
      setSavedFlash(true);
  ...
  const doTransfer = () => {
    ...
    update(prev => ({
      ...prev,
      garage: { ...prev.garage, [transferFor.id]: (prev.garage[transferFor.id] || 0) - actual },
      truck: { ...prev.truck, [transferFor.id]: (prev.truck[transferFor.id] || 0) + actual },
    }));
    setTransferFor(null);
    setTransferAmt('1');
  };
  ...
  if (!loaded) {
    return (
      <div style={{ background: THEME.bg, minHeight: '100vh' }} className="flex items-center justify-center">
        <div style={{ color: THEME.ink3, fontFamily: FONT.body, fontSize: 13, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
          Loading inventory
        </div>
      </div>
    );
  ...
  const accentSoft = location === 'garage' ? THEME.garageSoft : THEME.truckSoft;

  return (
    <div style={{ background: THEME.bg, minHeight: '100vh', fontFamily: FONT.body, color: THEME.ink }}>
      {/* Header */}
      <header className="px-5 pt-6 pb-4">
        <div className="flex items-start justify-between">
          <div>
            <div style={{ fontSize: 10, letterSpacing: '0.22em', color: THEME.ink3, textTransform: 'uppercase', fontWeight: 500 }}>
              Shield Pest Appalachia
            </div>
            <h1 style={{ fontFamily: FONT.display, fontSize: 28, fontWeight: 700, letterSpacing: '-0.02em', marginTop: 4, lineHeight: 1 }}>
              Field Daily
            </h1>
          </div>
          <div className="flex items-center gap-2">
            {savedFlash && (
              <div style={{ fontSize: 10, letterSpacing: '0.1em', color: THEME.garage, textTransform: 'uppercase', fontWeight: 600 }}>
                Saved
              </div>
            )}
            <button
        ...
        <div style={{ marginTop: 12, fontSize: 12, color: THEME.ink2, display: 'flex', alignItems: 'center', gap: 6 }}>
          <span>Technician</span>
          <span style={{ color: THEME.ink, fontWeight: 500 }}>Ashton</span>
        </div>
      </header>

      {/* Location toggle - the signature moment */}
      ...
      <main className="px-5 pb-8">
        {PRODUCT_CATEGORIES.map((cat, idx) => (
          <section key={cat.id} className={idx === 0 ? '' : 'mt-6'}>
            <div className="flex items-baseline justify-between mb-2 px-1">
              ...
              <div style={{ fontSize: 10, color: THEME.ink3, fontFamily: FONT.mono, letterSpacing: '0.05em' }}>
            ...
            >
              {cat.products.map((p, i) => {
                ...
                const otherCount = inv[location === 'garage' ? 'truck' : 'garage'][p.id] || 0;
        ...
        <section className="mt-6">
          ...
          <textarea
            ...
            placeholder="Anything else worth noting for this location..."
            rows={4}
            style={{
              width: '100%',
              background: THEME.canvas,
        ...
        <div style={{ marginTop: 24, textAlign: 'center', fontSize: 10, color: THEME.ink3, fontFamily: FONT.mono, letterSpacing: '0.08em' }}>
          AUTO-SAVED · SYNCED ACROSS SESSIONS
        </div>
      </main>
...
function ProductRow({ product, count, otherCount, status, location, accent, onInc, onDec, onSet, onTransfer, showDivider }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 15, fontWeight: 500, color: THEME.ink, letterSpacing: '-0.01em', lineHeight: 1.25 }}>
          {product.name}
        </div>
        {product.sub && (
          <div style={{ fontSize: 11, color: THEME.ink3, marginTop: 2 }}>{product.sub}</div>
        )}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6 }}>
          ...
          {location === 'truck' && (
            <div style={{ fontSize: 10, color: THEME.ink3, fontFamily: FONT.mono, letterSpacing: '0.03em' }}>
              GARAGE: {otherCount}
            </div>
          )}
          {location === 'garage' && (
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
        ...
        {location === 'truck' && (
          <button
            ...
            title={otherCount === 0 ? 'Nothing in garage to transfer' : 'Transfer from garage to truck'}
          >
...
function TransferModal({ product, garageCount, truckCount, amount, setAmount, onConfirm, onCancel }) {
    ...
    >
      ...
      >
        <div className="flex items-start justify-between mb-4">
          <div>
            <div style={{ fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', color: THEME.ink3, fontWeight: 600 }}>
              Transfer to Truck
            </div>
            <div style={{ fontSize: 20, fontWeight: 600, marginTop: 4, letterSpacing: '-0.01em' }}>{product.name}</div>
            {product.sub && <div style={{ fontSize: 12, color: THEME.ink3, marginTop: 2 }}>{product.sub}</div>}
          </div>
          <button
            onClick={onCancel}
            style={{ padding: 6, borderRadius: 8, color: THEME.ink2, background: 'transparent', border: 'none', cursor: 'pointer' }}
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 12, alignItems: 'center', marginBottom: 20 }}>
          <StockCard label="Garage" count={garageCount} accent={THEME.garage} soft={THEME.garageSoft} />
          <ArrowLeftRight size={20} strokeWidth={1.5} color={THEME.ink3} />
          <StockCard label="Silver Truck" count={truckCount} accent={THEME.truck} soft={THEME.truckSoft} />
        </div>

        {/* Amount input */}
        <div style={{ marginBottom: 16 }}>
          <label style={{ fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase', color: THEME.ink2, fontWeight: 600 }}>
            Amount to transfer
          </label>
          ...
          {willExceed && (
            <div style={{ marginTop: 8, fontSize: 12, color: THEME.zero, display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
              ...
              <span>
                ...
                <span style={{ color: THEME.garage, fontWeight: 700 }}>{garageCount - amt}</span>
              </span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <span>
                ...
                <span style={{ margin: '0 6px', color: THEME.ink3 }}>→</span>
                <span style={{ color: THEME.truck, fontWeight: 700 }}>{truckCount + amt}</span>
              </span>
            </div>
          </div>
        )}
...
function StockCard({ label, count, accent, soft }) {
  return (
    ...
    >
      <div style={{ fontSize: 9, letterSpacing: '0.14em', textTransform: 'uppercase', color: accent, fontWeight: 700 }}>
        {label}
      </div>
      <div
        style={{
...
function ResetModal({ onConfirm, onCancel }) {
  return (
    ...
    >
      ...
      >
        <div style={{ fontSize: 18, fontWeight: 600, marginBottom: 8 }}>Reset all counts?</div>
        <div style={{ fontSize: 14, color: THEME.ink2, marginBottom: 20, lineHeight: 1.5 }}>
